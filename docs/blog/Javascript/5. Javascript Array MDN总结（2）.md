...
at() 方法接收一个整数值并返回该索引对应的元素，允许正数和负数。负整数从数组中的最后一个元素开始倒数。如果没有找到指定的索引，则返回 undefined。
...
要返回的数组元素的索引（从零开始），会被转换为整数。负数索引从数组末尾开始计数——如果 index < 0，则会访问 index + array.length 处的元素。
...
### 32.Array.prototype.findLast()
findLast() 方法返回数组中满足提供的测试函数的最后一个元素的值。如果没有找到，则返回 undefined。它和 find() 一样，只是从数组的末尾开始向前遍历。
...
const arr = [1, 2, 3];
arr.findLast(x => x < 3); // 2 (注意不是 1，因为从后往前找)
arr.findLast(x => x > 10); // undefined
```

### 33.Array.prototype.findLastIndex()
findLastIndex() 方法返回数组中满足提供的测试函数的最后一个元素的索引。如果没有找到，则返回 -1。它和 findIndex() 一样，只是从数组的末尾开始向前遍历。
...
array1.findLastIndex(isLargeNumber); // 3 (130)
array1.findIndex(isLargeNumber);     // 2 (50)
```

### 34.Array.prototype.toReversed()
toReversed() 方法返回一个新数组，其元素顺序被反转（原数组不被修改）。与之相反，reverse() 会就地反转并返回原数组。
...
console.log(items);         // [1, 2, 3] (原数组未被修改)
```

### 35.Array.prototype.toSorted()
toSorted() 方法返回一个新数组，其元素按升序排序（原数组不被修改）。与之相反，sort() 会就地排序并返回原数组。

**语法**

```javascript
arr.toSorted([compareFunction(firstEl, secondEl)])
```
**示例**
...
toSpliced() 方法返回一个新数组，在原数组的某个位置删除、替换或插入元素（原数组不被修改）。与之相反，splice() 会就地修改原数组。
...
console.log(newMonths); // ["Jan", "Feb", "Mar", "Apr", "May"]
console.log(months);    // ["Jan", "Mar", "Apr", "May"]

const newArr = months.toSpliced(1, 2, "Feb", "Mar2");
console.log(newArr); // ["Jan", "Feb", "Mar2", "May"]
```

### 37.Array.prototype.with()
with() 方法返回一个新数组，其中指定索引处的元素被给定的值替换（原数组不被修改）。
...
// 支持负索引，从数组末尾开始计数
arr.with(-1, 9); // [1, 2, 3, 4, 9]

// 索引越界会抛出 RangeError
arr.with(10, 0); // RangeError: Invalid index : 10
```
...
values() 方法返回一个新的 Array Iterator 对象，它包含数组中每个索引的值。和 Object.values 不一样。
...
### 40.Array.prototype.toString()
toString() 方法返回一个字符串，表示指定的数组及其元素。数组的 toString() 方法实际上在内部调用了 join() 方法来拼接数组，并返回一个包含所有数组元素的逗号分隔的字符串。如果一个数组的元素是 null 或 undefined，它们会被转换为空字符串。
...
// 嵌套数组（toString 会递归调用，等价于深层 join）
const arr2 = [1, [2, 3], 4];
arr2.toString(); // "1,2,3,4"
```

### 41.Array.prototype.toLocaleString()
toLocaleString() 返回一个字符串表示数组中的元素。数组中的元素将使用各自的 toLocaleString 方法转成字符串，这些字符串将使用一个特定语言环境的字符串（例如逗号 ", "）隔开。
...
```javascript
const prices = [1000.5, 2000, 3000.75];
prices.toLocaleString('zh-CN', { style: 'currency', currency: 'CNY' });
// "¥1,000.50,¥2,000.00,¥3,000.75"

const dateArr = [new Date(2024, 0, 1), new Date(2024, 5, 15)];
dateArr.toLocaleString('zh-CN');
// "2024/1/1 00:00:00,2024/6/15 00:00:00"
...
### 42.Array.fromAsync()
Array.fromAsync() 静态方法从一个异步可迭代对象、同步可迭代对象或类数组对象创建一个新的数组。与 Array.from() 不同的是，它可以等待异步可迭代对象中的 Promise 完成。该方法是异步的，返回一个 Promise。
...
// 数组里包含 Promise，会等待 resolve
const arr = [Promise.resolve(1), Promise.resolve(2), 3];
Array.fromAsync(arr).then(array => console.log(array));
// [1, 2, 3]
```

### 43.Array.prototype[Symbol.iterator]()
[Symbol.iterator]() 方法返回一个迭代器对象，用于遍历数组的每个值。它是数组的默认迭代器，行为等同于 values() 方法。
...
### 补充：Array.prototype.group() / groupToMap()
`Array.prototype.group()` 和 `Array.prototype.groupToMap()` 曾作为提案出现，但最终在 ECMAScript 2024 中被替换为 **Object.groupBy()** 和 **Map.groupBy()**，并没有以 Array.prototype 上的方法落地。使用分组功能时请改用：
...
Map.groupBy(people, p => p.age > 20 ? 'adult' : 'minor');
// Map(2) { 'minor' => [...], 'adult' => [...] }
```
