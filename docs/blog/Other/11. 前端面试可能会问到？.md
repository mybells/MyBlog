...
### 11. Event Loop
  ...
  #### 事件循环的核心运行机制
  在浏览器或 Node.js 中，事件循环会持续运作，它的检查顺序极其严格：

  1. 先执行“主线程”同步代码（这是第一优先级）。

  2. 清空“微任务队列”（Microtask）：当同步代码执行完，事件循环会立即清空所有微任务（比如 Promise.then、MutationObserver），如果有新微任务产生，会继续清空，直到队列为空。

  3. 执行一个“宏任务”（Macrotask）：从宏任务队列（比如 setTimeout、setInterval、I/O 操作）中取出一个任务执行。

  4. 再次清空“微任务队列”：执行完这个宏任务后，会立刻再次清空所有微任务。

  5. 进入下一轮循环：接着去宏任务队列取下一个任务，周而复始。

  6. 记住口诀：“先同步，再微任务，后宏任务，且每执行一个宏任务，必清空一次微任务。”
